...
/**
 * @program:imexercise
 * @description:多线程回显处理器
 * @author: Huan Wang(https://github.com/njustwh2014)
 * @create:2020-04-08 12:19
 **/
public class MultiThreadEchoHandler implements Runnable {
    private static Logger logger=Logger.getLogger(MultiThreadEchoHandler.class);
    ...
    public MultiThreadEchoHandler(Selector selector, SocketChannel s) throws IOException {
        socketChannel=s;
    ...
    private synchronized void asyncRun() {
        logger.info("echo handler run on threadd: "+Thread.currentThread().getName());
        try{
            if(state==SENDING){
                //发送状态，buffer数据写入通道
